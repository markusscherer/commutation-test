...
test_all(const std::vector<array_function<3, 1, uint8_t>>& v1,
         const std::vector<array_function<3, 2, uint8_t>>& v2,
         const std::vector<array_function<3, 3, uint8_t>>& v3,
         const std::vector<array_function<3, 4, uint8_t>>& v4);

template <class Tester>
typename Tester::matches_type
test_ranges(const std::vector<array_function<3, 1, uint8_t>>& v1,
            const std::vector<array_function<3, 2, uint8_t>>& v2,
            const std::vector<array_function<3, 3, uint8_t>>& v3,
            const std::vector<array_function<3, 4, uint8_t>>& v4,
...
int main(int argc, char **argv) {
    ...
    if (!infilenames[0].empty()) {
        v1 = read_functions<D, 1>(infilenames[0]);
    }

    if (!infilenames[1].empty()) {
        v2 = read_functions<D, 2>(infilenames[1]);
    }

    if (!infilenames[2].empty()) {
        v3 = read_functions<D, 3>(infilenames[2]);
    }

    if (!infilenames[3].empty()) {
        v4 = read_functions<D, 4>(infilenames[3]);
    }
