...
template <> struct accumulating_result_handling_policy<4, 3, 3> {
    struct registers {
    ...
    inline static bool handle_results(__m128i resf, __m128i resg,
        ...
        if (matcount % 16 == 12) {
            brute_force_evaluation_policy<4, 3>::eval(
...
template <> struct accumulating_result_handling_policy<4, 3, 4> {
    struct registers {
        __m128i accmatfh = _mm_setzero_si128();
        __m128i accmatfl = _mm_setzero_si128();
        __m128i accmatgh = _mm_setzero_si128();
